...
models={}
for i in range(26):
  with open(f'/content/drive/MyDrive/Events/Mosaic/aphabets_model/model_{i}.pkl', 'rb') as f:
      # Load the parameters
      m = pickle.load(f)
      models[chr(97+i)]=m
import numpy as np
...
def suggest_next_letter_sol(displayed_word, guessed_letters):
  ...
  while(d/n<=0.6):
    ...
    scores=[]
    df=pd.read_csv("/content/drive/MyDrive/Events/Mosaic/aphabets_model/noprmalize.csv")
    for i in range(26):
